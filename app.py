...
# The app implements FIDE Laws of Chess (2023) via the python-chess library,
# which handles all rules including piece movements, check/checkmate, castling,
# en passant, promotion, stalemate, insufficient material, 50-move rule,
# threefold repetition, and other provisions from https://handbook.fide.com/chapter/e012023
...
def make_move():
    ...
    if move not in board.legal_moves:
        if not board.is_pseudo_legal(move):
            error = "Illegal move: that piece cannot move to that square."
        else:
            error = "Illegal move: it would leave your king in check."
        return jsonify({'error': error}), 400

    board.push(move)

    if board.is_game_over() or board.can_claim_draw():
        result_status = 'player_won' if board.result() == '1-0' else 'draw' if board.result() == '1/2-1/2' or board.can_claim_draw() else 'robot_won'
        return jsonify({
            'fen': board.fen(),
            'board': get_board_state(board),
            ...
            'message': f'Game over: {board.result() if not board.can_claim_draw() else "1/2-1/2"}.'
        })
    ...
    if board.is_game_over() or board.can_claim_draw():
        result = 'robot_won' if board.result() == '0-1' else 'draw' if board.result() == '1/2-1/2' or board.can_claim_draw() else 'player_won'
        message = f'Game over: {board.result() if not board.can_claim_draw() else "1/2-1/2"}.'
